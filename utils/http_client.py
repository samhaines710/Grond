...
def fetch_option_greeks(
    ...
    if results:
        # Filter by contract_type == typ if available
        same_type = [
            o for o in results
...
def fetch_option_greeks(
    ...
    if results:
        # Filter by contract_type == typ if available, else use all
        same_type = [
            o
            for o in results
            if o.get("details", {}).get("contract_type", "").lower() == typ
        ...
        # Pick ATM (closest strike to underlying price)
        opt = min(
            candidates,
            key=lambda o: abs(
                o["details"]["strike_price"]
    ...
    if len(bars) >= 2:
        ...
        if rets:
            m = sum(rets) / len(rets)
            v = sum((r - m) ** 2 for r in rets) / max(len(rets) - 1, 1)
            # Annualize: 78 5-min bars per day × 252 days/year
            sigma = math.sqrt(v * 78 * 252)
